...
public:
  Scanner();
  uint8_t attach(int pin, int pingPin);      // attach the given pin to the next free channel, sets pinMode, returns channel number or 0 if failure
...
private:
  ...
  int ping_pin;                       	// data pin for PING))) sensor
  int span;								//degrees
  int test_points;						//number of locations the scanner stops to take a meaurement
